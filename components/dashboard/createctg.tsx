...
export function CreateCtgForm({
  ...
  let createProdcut = async (e: FormEvent) => {
    ...
    let req;
    if (updateId) {
      req = await update("categories/" + updateId, formData);
    } else {
      req = await create("categories", formData);
    }
    ...
    <div className="fixed w-screen min-h-screen z-5 right-0 top-0 flex justify-end">
      ...
      >
        ...
        <div className="flex flex-col gap-2 w-full items-start">
          ...
          <input
            type="text"
            className="max-w-xl w-full py-2 px-2 border border-[#ddd] outline-purple7600"
            name="arabicName"
            value={formData.arabicName}
            onChange={(e) => updateFormValue("arabicName", e)}
            placeholder="Mac 16"
            required
          />
        </div>
